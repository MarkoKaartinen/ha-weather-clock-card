...
import { formatTemperature, forecastDate, iconName, isKnownCondition, windArrow, type Forecast } from "./weather-utils";
...
export class WeatherClockCard extends LitElement {
  ...
  protected updated(changed: Map<string, unknown>): void {
    if (changed.has("hass")) void this.subscribeData();
  }
    ...
    const subscribe = async (entity: string, forecastType: "hourly" | "daily", setter: (data: Forecast[]) => void) => {
      try {
        const unsubscribe = await this.hass!.connection.subscribeMessage(
          (message) => setter(message.forecast ?? []),
  ...
  private async subscribeCalendar(entity: string): Promise<void> {
    ...
    try {
      const unsubscribe = await this.hass.connection.subscribeMessage((message) => {
        const incoming = (message.event?.events ?? []) as CalendarEvent[];
  ...
  private renderForecast(item: Forecast, daily = false): TemplateResult {
    ...
    return html`<div class="forecast-item">
      ...
      ${daily ? nothing : html`<div class="wind">${windArrow(item.wind_bearing)} ${item.wind_speed ?? "—"}</div><div class="gust">💨 ${item.wind_gust_speed ?? "—"}</div>`}
    </div>`;
  }
    ...
    return html`<ha-card>
      <section class="current" part="current">
        <div class="current-copy">
          ...
          <div class="condition">${isKnownCondition(condition) ? condition?.replaceAll("-", " ") : `${this.t("unknown_condition", "Unknown condition")}: ${condition ?? "—"}`}</div>
          ...
          <div class="wind-details">${windArrow(attributes.wind_bearing as number | string)} ${attributes.wind_speed ?? "—"} ${attributes.wind_speed_unit ?? ""} <span>💨 ${attributes.wind_gust_speed ?? "—"} ${attributes.wind_speed_unit ?? ""}</span></div>
        </div>
        <img class="current-icon" src=${this.icon(condition)} alt=${condition ?? ""} />
      </section>
      ${this.config.show_calendar !== false ? html`<section class="calendar" part="calendar"><ha-icon icon="mdi:calendar-today"></ha-icon><div><strong>${this.t("today", "Today")}</strong>${this.events.length ? this.events.map((event) => html`<div>${this.eventTime(event)} ${event.summary ?? ""}</div>`) : html`<div class="muted">${this.t("no_events", "No events today")}</div>`}</div></section>` : nothing}
